//! Boot Axum + Leptos for the minimal Orbital SSR+hydrate host.

use axum::Router;
use leptos::config::get_configuration;
use leptos_axum::{generate_route_list, LeptosRoutes};
use minimal_ssr_hydrate::{shell, App};
use tower_http::services::ServeDir;

// `current_thread` keeps Leptos sandboxed-arena Owner cleanup on the same thread
// that created SendWrapper-backed effects (same pattern as orbital-preview-server).
#[tokio::main(flavor = "current_thread")]
async fn main() {
    simple_logger::init_with_level(log::Level::Info).expect("logger");

    // CSSTransition uses `spawn_local` during SSR.
    ...
    log::info!("minimal-ssr-hydrate listening on http://{addr}");
    let listener = tokio::net::TcpListener::bind(&addr)
        .await
        .expect("bind minimal-ssr-hydrate");
    axum::serve(listener, app.into_make_service())
        .await
        .expect("serve minimal-ssr-hydrate");
}
