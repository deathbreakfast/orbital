//! Boot Axum + Leptos for the server-paged-analytics host.

use axum::Router;
use leptos::config::get_configuration;
use leptos_axum::{generate_route_list, LeptosRoutes};
use server_paged_analytics::{shell, App};
...
async fn main() {
    ...
    log::info!("server-paged-analytics listening on http://{addr}");
    let listener = tokio::net::TcpListener::bind(&addr)
        .await
        .expect("bind server-paged-analytics");
    axum::serve(listener, app.into_make_service())
        .await
        .expect("serve server-paged-analytics");
}
